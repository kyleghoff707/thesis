#!/usr/bin/env python3
"""
Generate a Thes1s-branded PDF from a research markdown file.
...
def main():
    ...
    parser.add_argument('--title', default='Agent Harness Engineering for Thes1s v3')
    parser.add_argument('--subtitle', default='Research Artifact - May 2026')
    parser.add_argument('--stage-label', default='Research')
    args = parser.parse_args()

    pdf = Thes1sPDF(title=args.title, subtitle=args.subtitle, stage_label=args.stage_label)
    pdf.add_title_page(info_lines=['May 2, 2026', 'Thes1s v3 Pipeline Architecture'])
