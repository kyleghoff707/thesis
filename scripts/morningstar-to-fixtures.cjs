 ...
 *   Header:  TICKER_statement-type_Annual_Restated,Year1,...,YearN[,TTM]
 ...
 *
 * Usage:  node scripts/morningstar-to-fixtures.cjs [TICKER]
...
function parseStatement(csvContent) {
  ...
  // Row 0 = header: TICKER_type_Annual_Restated, Year1, ..., YearN [, TTM]
  const header = rows[0];
  const yearLabels = header.slice(1).map(y => y.trim());
  const hasTTM = yearLabels[yearLabels.length - 1] === 'TTM';
  const annualYears = hasTTM ? yearLabels.slice(0, -1) : [...yearLabels];
  ...
  for (let r = 1; r < rows.length; r++) {
    ...
    const hasAnyValue = row.slice(1, yearLabels.length + 1).some(v => {
      const s = (v || '').trim();
      return s !== '' && s !== '.';
    });
    ...
    for (let j = 0; j < yearLabels.length; j++) {
      ...
      if (val !== null) {
        data[year][fieldName] = val;
      }
    }
  }

  return { annualYears, hasTTM, data, fiscalYearEnd, currency };
}
...
function processCompany(ticker, dir) {
  ...
  for (const [filePattern, stmtType] of Object.entries(STATEMENT_TYPES)) {
    ...
    const firstYear = result.annualYears[0];
    stats[stmtType] = {
      years: result.annualYears.length,
      hasTTM: result.hasTTM,
      fields: firstYear ? Object.keys(result.data[firstYear] || {}).length : 0,
    };
  }
...
function validateFixture(ticker, fixture) {
  ...
  const incYears = Object.keys(fixture.statements.income || {}).filter(y => y !== 'TTM');
  const bsYears = Object.keys(fixture.statements.balance_sheet || {}).filter(y => y !== 'TTM');
  const cfYears = Object.keys(fixture.statements.cash_flow || {}).filter(y => y !== 'TTM');

  if (incYears.length > 0 && bsYears.length > 0) {
    const incSet = new Set(incYears);
    const bsSet = new Set(bsYears);
    for (const y of bsYears) {
      if (!incSet.has(y)) warnings.push(`BS year ${y} not in IS`);
    }
  }

  // Spot-check known values (AAPL 2024 revenue)
  if (ticker === 'AAPL') {
    const rev = fixture.statements.income?.['2024']?.['Total Revenue'];
    if (rev !== 391035000000) {
      warnings.push(`AAPL 2024 revenue: expected 391035000000, got ${rev}`);
    }
    const ni = fixture.statements.income?.['2024']?.['Net Income Available to Common Stockholders'];
    if (ni !== 93736000000) {
      warnings.push(`AAPL 2024 net income: expected 93736000000, got ${ni}`);
    }
    const ta = fixture.statements.balance_sheet?.['2024']?.['Total Assets'];
    if (ta !== 364980000000) {
      warnings.push(`AAPL 2024 total assets: expected 364980000000, got ${ta}`);
    }
    const cfo = fixture.statements.cash_flow?.['2024']?.['Cash Flow from Operating Activities, Indirect'];
    if (cfo !== 118254000000) {
      warnings.push(`AAPL 2024 CFO: expected 118254000000, got ${cfo}`);
    }
  }

  // Spot-check LULU (non-calendar FY)
  if (ticker === 'LULU') {
    const rev = fixture.statements.income?.['2025']?.['Total Revenue'];
    if (rev !== 10588126000) {
      warnings.push(`LULU 2025 revenue: expected 10588126000, got ${rev}`);
    }
  }

  // Spot-check BRK-B (conglomerate)
  if (ticker === 'BRK-B') {
    const rev = fixture.statements.income?.['2024']?.['Total Revenue'];
    if (rev !== 424232000000) {
      warnings.push(`BRK-B 2024 revenue: expected 424232000000, got ${rev}`);
    }
  }
...
function main() {
  ...
  if (requestedTicker) {
    ...
    if (!fs.existsSync(dir)) {
      console.error(`No Morningstar data found for ${requestedTicker}`);
      process.exit(1);
    }
  ...
  console.log(`\nMORNINGSTAR CSV → JSON FIXTURE CONVERTER`);
  ...
  for (const ticker of tickers) {
    ...
    try {
      ...
      for (const [st, s] of Object.entries(stats)) {
        ...
        parts.push(`${label}:${s.fields}f`);
      }
      parts.push(`${stats.income?.years || '?'}yr`);
