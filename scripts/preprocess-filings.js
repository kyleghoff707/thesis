...
let ticker;
try {
  ticker = normalizeTicker(process.argv[2]);
} catch (error) {
...
async function main() {
  ...
  console.log(`Pre-processing ${annuals.length} 10-Ks (max 5) and ${quarterly.length} 10-Qs (max 4) for ${ticker}...`);

  // Create output directory
  const outDir = join(reportsDir(ticker), 'filings-md');
