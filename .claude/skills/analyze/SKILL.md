...
Run the complete Thesis investment analysis pipeline using a free Thesis Data API key and Claude Code subagents.
...
All AI work runs as Claude Code subagents (Pro Max subscription compute). Financial DataPackets come from the hosted Thesis Data API.
...
### No Claude API Calls
All AI work must run as Claude Code subagents via the existing generation skills. Never call `run-pipeline.js` or the Claude API directly. The user's Claude Code subscription covers subagent compute. The only required hosted call is the Thesis Data API call performed by `assemble-data.js` or `prepare-data.js`.
