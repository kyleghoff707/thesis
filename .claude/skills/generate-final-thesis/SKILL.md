...
```bash
node scripts/update-status.js {TICKER} finalThesis IN_PROGRESS
```
...
## Step 11.5: Mark Stage Complete

```bash
VERDICT=$(node -e "console.log(JSON.parse(require('fs').readFileSync(require('os').homedir() + '/thesis/reports/{TICKER}/final-thesis.json','utf8')).overallVerdict)")
node scripts/update-status.js {TICKER} finalThesis COMPLETED "$VERDICT"
```
