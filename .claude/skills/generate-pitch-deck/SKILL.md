...
Runs gate check, initializes local generation status, fetches the DataPacket from the hosted Thesis Data API, loads bundled transcripts from `./transcripts/`, pre-processes SEC filing sections locally, and runs the data quality checkpoint.
...
```bash
node scripts/update-status.js {TICKER} pitchDeck IN_PROGRESS
```
...
const reportPath = path.join(os.homedir(), 'thesis', 'reports', '{TICKER}', 'pitch-deck.json');
const report = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
const sections = Array.isArray(report.sections) ? report.sections : [];
const missingNarrative = sections.filter(s => !String(s.narrative || '').trim()).map(s => s.key);
const missingVerdict = sections.filter(s => !String(s.verdict || '').trim()).map(s => s.key);
console.log('Quality check complete.');
console.log('Sections:', sections.length);
console.log('Missing narrative:', missingNarrative.length ? missingNarrative.join(', ') : 'none');
console.log('Missing verdict:', missingVerdict.length ? missingVerdict.join(', ') : 'none');
if (missingNarrative.length || missingVerdict.length) process.exit(1);
"
```

Quality is a local completeness gate. If it fails, fix the affected section JSON before continuing.
...
const os = require('os');
const path = require('path');
const reportDir = path.join(os.homedir(), 'thesis', 'reports', '{TICKER}');
const report = JSON.parse(fs.readFileSync(path.join(reportDir, 'pitch-deck.json'), 'utf8'));
const summary = (report.sections || []).reduce((acc, section) => {
  const tokenCost = section.tokenCost || {};
  acc.input += tokenCost.input || 0;
  acc.output += tokenCost.output || 0;
  return acc;
}, { input: 0, output: 0 });
const budgetPath = path.join(reportDir, 'budget.json');
const existing = fs.existsSync(budgetPath) ? JSON.parse(fs.readFileSync(budgetPath, 'utf8')) : {};
fs.writeFileSync(budgetPath, JSON.stringify({ ...existing, pitchDeck: summary }, null, 2));
console.log('Pitch Deck token summary:', summary);
"
```
...
## Step 17.5: Mark Stage Complete

```bash
VERDICT=$(node -e "console.log(JSON.parse(require('fs').readFileSync(require('os').homedir() + '/thesis/reports/{TICKER}/pitch-deck.json','utf8')).overallVerdict)")
node scripts/update-status.js {TICKER} pitchDeck COMPLETED "$VERDICT"
```
