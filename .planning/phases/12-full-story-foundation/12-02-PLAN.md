...
must_haves:
  truths:
    ...
    - "Risk-analyst prompt has a Debate Step 2: Bear Inversion section using the lightweight BearInversion format (not the monolithic S6 ReportSectionSchema)"
    - "Management-evaluator S4 data field uses the unified checklist schema: items (not checklist), verdict PASS|FAIL|PARTIAL (not assessment PASS|FAIL|REVIEW|INSUFFICIENT_DATA), number (not point), item (not question), checklistType, summary.scoreDisplay"
  artifacts:
    ...
    - path: "agents/risk-analyst/prompt.md"
      provides: "Full Story debate bear inversion role instructions using lightweight BearInversion format"
      contains: "Debate Step 2: Bear Inversion"
    - path: "agents/management-evaluator/prompt.md"
      provides: "Full Story S4 management_checklist with unified checklist schema"
  ...
  key_links:
    ...
    - from: "agents/orchestrator/schemas/checklist-item.schema.json"
      to: "agents/management-evaluator/prompt.md"
      via: "S4 data field updated to match unified schema"
      pattern: "checklistType.*management.*items.*verdict.*PASS.*FAIL.*PARTIAL"
    ...
    - from: "agents/orchestrator/schemas/debate-step.schema.json"
      to: "agents/risk-analyst/prompt.md"
      via: "bear inversion output format matches BearInversion schema"
      pattern: "inversions.*targetPoint.*counterArgument.*severity.*thesis_killer"
    - from: "agents/orchestrator/schemas/debate-step.schema.json"
...
<objective>
Add Full Story-specific section instructions to the 7 agent prompts that need them -- 5 new prompts (business-analyst, competitor-evaluator, valuation-specialist, synthesis-writer, financial-analyst) plus targeted updates to 2 existing prompts (risk-analyst, management-evaluator).

Purpose: Each agent needs to know exactly what to produce when dispatched for a Full Story section. The risk-analyst and management-evaluator already have Full Story instructions from Phase 06, but those instructions need updates: the risk-analyst needs a new lightweight "Debate Step 2: Bear Inversion" section for the 4-step debate format (the existing monolithic S6 instructions were designed for a single-agent ReportSectionSchema, not the debate), and the management-evaluator's S4 data format needs alignment with the unified checklist-item.schema.json from Plan 01 (field names and verdict values diverge).

Output: 7 updated agent prompt files, each with correct Full Story section blocks.
</objective>
...
From agents/management-evaluator/prompt.md (CURRENT S4 format -- NEEDS UPDATE):
```markdown
...
```json
{
  "checklist": [               <-- WRONG: should be "items" per unified schema
    {
      "point": 1,              <-- WRONG: should be "number"
      "question": "...",       <-- WRONG: should be "item"
      "evidence": "...",
      "assessment": "PASS | FAIL | REVIEW | INSUFFICIENT_DATA",  <-- WRONG: should be "verdict": "PASS | FAIL | PARTIAL"
      "confidence": "HIGH | MEDIUM | LOW"
    }
  ],
  "checklistScore": "X/13",   <-- WRONG: should be summary.scoreDisplay
  "passingPoints": 0,          <-- WRONG: should be summary.passCount
  "failingPoints": 0,          <-- WRONG: should be summary.failCount
  "reviewPoints": 0,           <-- WRONG: should be summary.partialCount
  "insufficientDataPoints": 0  <-- REMOVE: not in unified schema
}
```
```

From agents/orchestrator/schemas/checklist-item.schema.json (CORRECT unified format from Plan 01):
- checklistType: "meaning" | "moat" | "management"
- items: [{ number, item, verdict: PASS|FAIL|PARTIAL, evidence, confidence: HIGH|MEDIUM|LOW }]
- summary: { passCount, failCount, partialCount, totalItems, scoreDisplay }

From agents/orchestrator/schemas/debate-step.schema.json (lightweight debate formats from Plan 01):
...
<task type="auto">
  ...
  <done>Valuation-specialist, synthesis-writer, and financial-analyst prompts updated with Full Story section and debate role instructions.</done>
</task>

<task type="auto">
  <name>Task 3: Update risk-analyst bear debate section and management-evaluator unified checklist schema</name>
  <files>
    agents/risk-analyst/prompt.md,
    agents/management-evaluator/prompt.md
  </files>
  ...
  <action>
    Two targeted updates to existing prompts that already have Full Story sections from Phase 06 but whose formats now diverge from the Plan 01 schemas.

    **risk-analyst/prompt.md -- Add "Debate Step 2: Bear Inversion" section:**

    The existing risk-analyst prompt has a monolithic "Full Story Section 6: Inversion and Rebuttal" section that outputs a full ReportSectionSchema. The new 4-step debate format (D-06, D-12) requires a separate lightweight bear inversion step. The existing S6 section can remain (it still serves as reference for the risk-analyst's overall approach) but a NEW section must be added for the debate role specifically.

    Append AFTER the existing "Full Story Section 6: Inversion and Rebuttal" section (before the Red Flag Mandate section) a new section:
    ...
    **management-evaluator/prompt.md -- Update S4 data format to match unified checklist schema:**

    Find the existing "Full Story Section 4: Management (13-Point Checklist)" section. Update the `data` field JSON example and surrounding instructions to align with the unified checklist-item.schema.json from Plan 01:

    1. Replace `"checklist": [...]` with `"items": [...]`
    2. Replace `"point": 1` with `"number": 1`
    3. Replace `"question": "..."` with `"item": "..."`
    4. Replace `"assessment": "PASS | FAIL | REVIEW | INSUFFICIENT_DATA"` with `"verdict": "PASS | FAIL | PARTIAL"` -- note: REVIEW and INSUFFICIENT_DATA from the old format map to PARTIAL in the unified schema (evidence is mixed or insufficient = PARTIAL)
    5. Add `"checklistType": "management"` as a top-level field in the data object
    6. Replace the flat score fields (`checklistScore`, `passingPoints`, `failingPoints`, `reviewPoints`, `insufficientDataPoints`) with the unified `"summary"` object:
       ```json
       "summary": {
         "passCount": 10,
         "failCount": 1,
       ...
       ```
    7. Update the verdict logic text: replace references to "REVIEW" and "INSUFFICIENT_DATA" with "PARTIAL" -- e.g., "PASS: 10+/13 points pass" stays the same, but "Too many INSUFFICIENT_DATA points" becomes "Too many PARTIAL items with LOW confidence"
    8. Update the "For each point, provide" text: change "Your assessment (PASS/FAIL/REVIEW/INSUFFICIENT DATA)" to "Your verdict (PASS/FAIL/PARTIAL)"

    The updated data field example should look like:
    ```json
    {
    ...
    Preserve ALL other content in the management-evaluator prompt (Pitch Deck Section 6 instructions, curriculum, DataPacket slice, citation enforcement, web search requirements, etc.). Only the S4 data format section changes.
  </action>
  <verify>
    <automated>node -e "
      const fs = require('fs');
      const ra = fs.readFileSync('agents/risk-analyst/prompt.md', 'utf8');
      const me = fs.readFileSync('agents/management-evaluator/prompt.md', 'utf8');

      // Risk-analyst: verify NEW debate bear section exists
      console.assert(ra.includes('Debate Step 2: Bear Inversion'), 'RA missing Debate Step 2: Bear Inversion heading');
      console.assert(ra.includes('role.*bear') || ra.includes('\"role\": \"bear\"') || ra.includes('role: \"bear\"'), 'RA missing bear role in output format');
      console.assert(ra.includes('inversions'), 'RA missing inversions array in debate format');
      console.assert(ra.includes('targetPoint'), 'RA missing targetPoint field');
      console.assert(ra.includes('counterArgument'), 'RA missing counterArgument field');
      console.assert(ra.includes('severity'), 'RA missing severity field');
      console.assert(ra.includes('thesis_killer'), 'RA missing thesis_killer severity value');
      console.assert(ra.includes('overallBearCase'), 'RA missing overallBearCase field');
      console.assert(ra.includes('lightweight debate'), 'RA missing lightweight debate format reference');
      // Verify existing S1 and S6 sections still present
      console.assert(ra.includes('Full Story Section 1: Event Analysis'), 'RA lost existing S1 section');
      console.assert(ra.includes('Full Story Section 6: Inversion and Rebuttal'), 'RA lost existing S6 section');
      console.assert(ra.includes('PEST'), 'RA lost existing PEST section');

      // Management-evaluator: verify unified checklist schema
      console.assert(me.includes('checklistType'), 'ME missing checklistType');
      console.assert(me.includes('\"management\"') || me.includes('management'), 'ME missing management checklistType value');
      console.assert(me.includes('scoreDisplay'), 'ME missing scoreDisplay');
      // Verify OLD format is gone
      console.assert(!me.includes('\"checklist\": [') && !me.includes('\"checklist\":['), 'ME still has old checklist array name');
      console.assert(!me.includes('INSUFFICIENT_DATA'), 'ME still has old INSUFFICIENT_DATA verdict');
      console.assert(!me.includes('checklistScore'), 'ME still has old checklistScore field');
      console.assert(!me.includes('insufficientDataPoints'), 'ME still has old insufficientDataPoints');
      // Verify NEW format is present
      console.assert(me.includes('\"items\"') || me.includes('items'), 'ME missing items array');
      console.assert(me.includes('PARTIAL'), 'ME missing PARTIAL verdict');
      console.assert(me.includes('passCount'), 'ME missing passCount in summary');
      // Verify existing Pitch Deck section preserved
      console.assert(me.includes('Pitch Deck Section 6'), 'ME lost existing Pitch Deck section');
      console.assert(me.includes('Guru Ownership Assessment'), 'ME lost existing Guru section');

      console.log('All risk-analyst bear debate and management-evaluator schema assertions passed');
    "
    </automated>
  </verify>
  <acceptance_criteria>
    - agents/risk-analyst/prompt.md contains "### Debate Step 2: Bear Inversion (role: \"bear\")" section
    - agents/risk-analyst/prompt.md bear section specifies lightweight debate format (NOT ReportSectionSchema)
    - agents/risk-analyst/prompt.md bear section output format includes inversions array with targetPoint, counterArgument, evidence, severity, sources fields
    - agents/risk-analyst/prompt.md bear section specifies severity enum: thesis_killer, significant, minor
    - agents/risk-analyst/prompt.md bear section explicitly states web search is enabled for this role (per D-07)
    - agents/risk-analyst/prompt.md bear section includes required web searches for the debate step
    - agents/risk-analyst/prompt.md retains ALL existing content (S1 event_analysis, S6 inversion_rebuttal, PEST, curriculum, etc.)
    - agents/management-evaluator/prompt.md S4 data field uses "items" (not "checklist")
    - agents/management-evaluator/prompt.md S4 data field uses "number" (not "point")
    - agents/management-evaluator/prompt.md S4 data field uses "item" (not "question")
    - agents/management-evaluator/prompt.md S4 data field uses verdict "PASS | FAIL | PARTIAL" (not "assessment" with REVIEW/INSUFFICIENT_DATA)
    - agents/management-evaluator/prompt.md S4 data field has "checklistType": "management"
    - agents/management-evaluator/prompt.md S4 data field has summary object with passCount, failCount, partialCount, totalItems, scoreDisplay
    - agents/management-evaluator/prompt.md retains ALL existing content (Pitch Deck Section 6, curriculum, DataPacket slice, etc.)
  </acceptance_criteria>
  <done>Risk-analyst prompt has new lightweight bear debate section for the 4-step debate format. Management-evaluator S4 data format aligned with the unified checklist-item.schema.json (items/verdict/checklistType/summary.scoreDisplay).</done>
</task>

</tasks>

<verification>
1. All 7 prompt files parse as valid markdown (no broken formatting)
2. Every agent that appears in the dispatch table's fullStory section has corresponding instructions in its prompt.md
3. Checklist prompts (business-analyst, competitor-evaluator, management-evaluator) ALL use the same unified schema: checklistType, items[{number, item, verdict: PASS|FAIL|PARTIAL, evidence, confidence}], summary{passCount, failCount, partialCount, totalItems, scoreDisplay}
4. Debate prompts (synthesis-writer, risk-analyst, financial-analyst) reference the same schema structure as debate-step.schema.json
5. Risk-analyst prompt contains "Debate Step 2: Bear Inversion" section with lightweight BearInversion format (targetPoint, counterArgument, evidence, severity, sources, overallBearCase) -- NOT just the old monolithic S6 section
6. Management-evaluator S4 data field does NOT contain old field names (checklist, point, question, assessment, checklistScore, insufficientDataPoints) -- only unified schema field names
7. Only the bear role (risk-analyst Debate Step 2) mentions web search being available for the debate -- bull, bull_rebuttal, and judge explicitly state they do NOT have web search
8. All prompts follow the same structural pattern: heading, purpose, data field, narrative requirements, verdict logic, red flags
</verification>

<success_criteria>
- All 7 agents have Full Story instructions (5 new from Tasks 1-2 + 2 updated from Task 3)
- All 3 checklist sections (meaning, moat, management) output the SAME unified scoring format (PASS/FAIL/PARTIAL with evidence and confidence, checklistType, summary.scoreDisplay)
- All 4 debate roles output the lightweight format (not full ReportSectionSchema)
- Risk-analyst bear debate section uses BearInversion format with web search enabled
