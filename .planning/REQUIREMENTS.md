...
- [ ] **FIX-01**: User can view Pitch Deck sections correctly after pipeline run (fix section key mismatch — 5 of 10 keys don't match pipeline output)
- [ ] **FIX-02**: User can store reports for 20+ companies without hitting browser storage limits (migrate report storage from localStorage to IndexedDB)
- [ ] **FIX-03**: User can load Full Story reports in the app (add Full Story route to Vite middleware)
...
- [ ] **INFRA-01**: User sees consistently formatted numbers, currencies, and percentages across all report stages (extract shared formatting utilities)
...
- [ ] **FS-01**: User can view Full Story report with 6 sections, gate check enforcing Pitch Deck approval, and approval bar
...
- [ ] **FS-03**: User can view the adversarial debate (Bull → Bear → Bull Rebuttal → Judge) with distinct visual treatment per step
- [ ] **FS-04**: User can see quality scores (mechanical and methodology) per section and overall
- [ ] **FS-05**: User can navigate between debate steps via tabs or accordion controls
