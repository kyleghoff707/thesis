...
- **In-app rendering**: Top nav bar shows logo mark (22px) + styled text where "Thes" is primary text, "1" is accent teal italic, "s" is primary text
...
- **Cache Storage**: Three-tier caching — in-memory Map (instant) → IndexedDB (`thes1s-cache` database, 6 object stores: `edgar-facts`, `edgar-statements`, `guru-data`, `nport-data`, `filing-markdown`, `insider-data`) → localStorage (small keys only). Large cache entries (EDGAR facts 500KB-10MB, financial statements 200-500KB, guru 13F data 200KB-3MB, N-PORT data 50-500KB, insider Form 4 data 1-50KB per filing) route to IndexedDB automatically via key-prefix routing. Small entries (CUSIP map, company details, series-match flags) stay in localStorage. `cacheStore.js` handles IndexedDB persistence with `fetchedAt` timestamps on all entries for "last refreshed" UI. Feature detection (`typeof indexedDB !== 'undefined'`) ensures Node.js validation scripts still work.
...
- **Navigation restructured** — Left sidebar removed, replaced with horizontal top nav bar (52px). Four top-level tabs: Watchlists | Research | Gurus | Reports, each with 14px SVG icons. Settings gear icon (far left), logo + brand (left), search bar (pushed right via `marginLeft: auto`). Routes: `/watchlists`, `/research`, `/research/:id` (was `/research/:id/toolbox`), `/gurus`, `/gurus/:cik` (individual guru portfolio detail view), `/reports`, `/validation`, `/guru-audit`, `/ticker-audit`, `/nport-audit`. Default landing page is `/research` (empty state). Validation, Guru Audit, Ticker Audit, and N-PORT Audit accessed via Settings modal "Tools" section. Old `/research/:id/toolbox` URLs redirect to `/research/:id`.
...
- Dependencies installed: recharts, @anthropic-ai/sdk, uuid, react-router-dom, turndown, turndown-plugin-gfm
...
│   ├── Toolbox.jsx           ✅ main toolbox container (tab navigation: Overview/Scores/Financials/Growth/Valuation/Filings/Insiders, fetches data, runs calcs). Settings sync via useEffect (no scroll reset). Opacity dimming during refetch. Filings and Insiders tabs render independently of edgarStatements gate.
...
│   ├── Valuation.jsx         ✅ valuation tab container — four sub-tabs (Growth Rate Analysis/Inputs/Calculators/Price vs Value), manages all shared valuation state (EPS, FGR, maintenance %, excluded years, FCF ratio, compositeMetrics, heroEnabled + Equity Bond state vars: ebBvpsOverride, ebRoeOverride, ebRetainedRatioOverride, ebAvgPEOverride + shared MARR). Also retains Method A Pretax + Bond Comparison state (ptebPretaxEPSOverride, ptebGrowthRateOverride, ptebPEOverride, ptebCorpBondYield, ebTBillYield, ebCorpBondYield) in codebase but not wired to UI. Computes Equity Bond defaults from EDGAR (BVPS, avg ROE, retained ratio, avg PE from mean daily close). Computes both `computeHistoricalHighPE` (for MOS cap) and `computeHistoricalAvgPE` (for Equity Bond) from full price history. Builds growth analysis series + weighted averages + composite GR, runs calculator engines (10 Cap, MOS, PBT, Equity Bond BVPS Growth). Save/load includes all override fields + heroEnabled. Calls usePrices(ticker, 'max') internally.
│   ├── ValuationCalculators.jsx ✅ hero box (highest buy price across enabled methods — shows "Future Price (10yr)" instead of "Sticker Price" when Equity Bond is hero) + 4-column summary row (10 Cap, MOS, PBT, Equity Bond) each with **checkbox to include/exclude from hero calculation** (unchecked dims card, excludes from hero, persisted in save) + 4 calculator sections: 10 Cap, MOS, PBT, Equity Bond (5 editable inputs + 6 locked outputs including Equity Growth Rate and Future BVPS). FGR radio group (Analyst/Composite/Custom) shared across MOS+PBT. MARR shared across MOS+PBT+EB. Shows PBT at current price. "Save Valuation" button at bottom (persists all overrides to localStorage).
│   ├── ValuationInputs.jsx   ✅ 4 historical data tables: 10 Cap Inputs (4 rows × years + TTM in millions), MOS Inputs (EPS + High PE per year), PBT Inputs (5 rows including per-year FCF/NI ratio with click-to-exclude year columns for outlier removal), BVPS Growth Inputs (7 rows: BVPS, ROE, EPS Diluted, Dividends Per Share, Retained Earnings Ratio, High P/E, Avg P/E). Accepts `returns` + `historicalAvgPE` props.
...
│   ├── Settings.jsx          ✅ Settings modal — theme, financial statement defaults (layout, version, view, periods), growth chart years, price range. Gurus section with N-PORT data toggle (`enableNport`). Tools section with 4 buttons: "Run Validation" (→ /validation), "Run Guru Audit" (→ /guru-audit), "Run Ticker Audit" (→ /ticker-audit), "Run N-PORT Audit" (→ /nport-audit). Save button with icon. Auto-saves on each change.
...
│   ├── cacheStore.js         ✅ IndexedDB persistence layer for large cache data (`thes1s-cache` database, 6 object stores: edgar-facts, edgar-statements, guru-data, nport-data, filing-markdown, insider-data). DB_VERSION=3. Follows priceStore.js pattern. Exports: idbGet, idbSet, idbGetMeta, idbBulkGet, idbClear. Feature detection for Node.js compat.
...
- **Toolbox navigation**: Horizontal underline tabs (Overview | Scores | Financials | Growth | Valuation | Filings | Insiders) — one section at a time, not stacked (local state within Research tab)
