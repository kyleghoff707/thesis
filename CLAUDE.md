...
│   ├── Toolbox.jsx           ✅ main toolbox container (tab navigation: Overview/Scores/Financials/Growth/Filings/Insiders, fetches data, runs calcs). Settings sync via useEffect (no scroll reset). Opacity dimming during refetch. Filings and Insiders tabs render independently of edgarStatements gate.
...
- **Toolbox navigation**: Horizontal underline tabs (Overview | Scores | Financials | Growth | Filings) — one section at a time, not stacked (local state within Research tab)
