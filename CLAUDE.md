...
- **Price Storage**: IndexedDB via `idb` npm package — persistent local price database. First lookup fetches full history from Yahoo, stores locally. Subsequent lookups serve from local store with incremental updates for new trading days. Works offline for previously viewed tickers.
...
- **Navigation restructured** — Left sidebar removed, replaced with horizontal top nav bar (52px). Four top-level tabs: Watchlists | Research | Gurus | Reports, each with 14px SVG icons. Settings gear icon (far left), logo + brand (left), search bar (pushed right via `marginLeft: auto`). Routes: `/watchlists`, `/research`, `/research/:id` (was `/research/:id/toolbox`), `/gurus`, `/gurus/:cik` (individual guru portfolio detail view), `/reports`, `/validation`, `/guru-audit`. Default landing page is `/research` (empty state). Validation and Guru Audit accessed via Settings modal "Tools" section. Old `/research/:id/toolbox` URLs redirect to `/research/:id`.
...
├── App.jsx                   ✅ Layout + Routes (/, /watchlists, /research, /research/:id, /reports, /gurus, /validation, /guru-audit), theme/research state
...
│   ├── GuruAudit.jsx         ✅ in-app guru validation (route: /guru-audit) — runs auditGurus() against EDGAR, progress bar, summary stats, issues in red / clean in green. Accessible via Settings → Tools.
│   ├── Toolbox.jsx           ✅ main toolbox container (tab navigation: Overview/Scores/Financials/Growth, fetches data, runs calcs). Settings sync via useEffect (no scroll reset). Opacity dimming during refetch.
...
│   ├── Settings.jsx          ✅ Settings modal — theme, financial statement defaults (layout, version, view, periods), growth chart years, price range. Tools section with "Run Validation" link. Save button with icon. Auto-saves on each change.
│   ├── Validation.jsx        ✅ Layer 1 validation page — batch runner, results display, aggregate summary, export JSON, "Include Quarterly Roll-Up" checkbox
...
│   ├── gurus.js              ✅ SEC EDGAR 13F engine — 43 guru CIKs, multi-filing fetch (current + previous quarter), change detection by CUSIP (new/added/reduced/sold/held), activity aggregation, top buys + top holdings (`aggregateTopHoldings`). History fetch (up to 8 quarters on-demand). Portfolio value history (`fetchPortfolioValueHistory`, up to 20 quarters). Ticker resolution via fuzzy issuer name matching + persistent CUSIP→ticker cache. Per-filing cache (immutable, versioned `GURU_CACHE_V`), submissions cache, activity cache, portfolio history cache. Namespace-aware XML parsing (`getElementsByTagNameNS`), auto-detects value convention (dollars vs thousands). **Options filtered** (`putCall` check — excludes puts/calls, equity only). **Share class aggregation** (`aggregateShareClasses` — merges by 6-char CUSIP prefix). **InfoTable detection** (3-tier: type field → filename → XML fallback, shared `findInfoTableFile` helper). `auditGurus()` export for in-app validation. Vite proxy in dev.
...
└── hooks/
    ...
    ├── useGurus.js           ✅ React hook for guru 13F data. Exposes: fetchOne, fetchOneWithChanges, fetchAll, fetchAllChanges, searchStock, fetchHistory, fetchPortfolioHistory. Activities state for Latest tab with latestTabData derived memo (sorted activities + top buys + top holdings). Hydrates both portfolio and activity caches on mount. Ticker resolution wired into change-detection fetch paths.
    ...
    ├── useSettings.js        ✅ app-wide settings (localStorage persistence, DEFAULT_SETTINGS export)
...
- **Toolbox navigation**: Horizontal underline tabs (Overview | Scores | Financials | Growth) — one section at a time, not stacked (local state within Research tab)
